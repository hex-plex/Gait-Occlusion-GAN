...
def fetch_data(subject='id001',noFrame=True , preFrame=True, returnName=False):
    assert len(subject)>=5
    subject = "static_occ_%s_6" % subject
...
def supervision(kmeans, angle_subject=None, override=False):
    ...
    if angle_subject is None:
        ...
        frames_name = []
        outliers = []
        for folder in tqdm(sorted(os.listdir(os.getcwd()+'/tumiitgait/Static Occlusion'))):
            if folder[-3:] == "avi":
                continue
            if override or not os.path.isfile('/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,'labels.pkl'])):
                kmean = kmeans['90']
                for file in sorted(os.listdir( '/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder]))):
                    if file[-3:]!="pkl":
                        frames_name.append(file)
                        frames.append(cv2.imread( '/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,file])))
                if len(frames) < 10:
                    outliers.append('/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder]))
                else:
                    try:
                        ...
                        with open('/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,'labels.pkl']),'wb') as handle:
                            pickle.dump(info,handle,protocol=pickle.HIGHEST_PROTOCOL)
                    except Exception as e:
                        print('/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder]))
                        raise Exception(e)
                frames_name = []
                frames = []
        print(outliers)
...
#     for option in os.listdir('/'.join([os.getcwd(), 'tumiitgait/Static Occlusion',subject])):
#         folder_exist = (option.find(choice[0])>=0 and option.find(choice[1])>=0)
#         if folder_exist:
#             choice = option
#             break

    if os.path.exists( '/'.join([os.getcwd(),'tumiitgait/Static Occlusion',subject])):
        if override or not os.path.isfile('Kmeans_weights.npz'):
            kmeans = {}
            km = KMeans()
            imgs = []
            for file in sorted(os.listdir('/'.join([os.getcwd(), 'tumiitgait/Static Occlusion', subject]))):
                if file[-3:]!="pkl":
                    imgs.append(cv2.imread('/'.join([os.getcwd(),'tumiitgait/Static Occlusion',subject,file]))) 
...
def fetch_labels(label_angle='090',filename="labels_cache",save=True,override=False):
    ...
    if override or not os.path.isfile(os.getcwd()+'/'+filename+'.npz'):
        ...
        for folder in tqdm(sorted(os.listdir(os.getcwd()+'/tumiitgait/Static Occlusion'))):
            ...
            label_file = '/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,'labels.pkl'])
            if os.path.isfile(label_file):
                labels_temp = pickle.load(open(label_file, 'rb'))
                for file in sorted(os.listdir('/'.join([os.getcwd(),'tumiitgait/Static Occlusion', folder]))):
                    if file[-3:]!="pkl" and file[-3:]!="npz" and file[-3:]!="npy":
                        labels['/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,file])] = labels_temp[file]
        if save:
...
def encode_data(encoder,label_angle='090',save=True,override=False):
    ...
    for folder in tqdm(sorted(os.listdir(os.getcwd()+'/tumiitgait/Static Occlusion'))):
        if folder[-3:] == "avi":
            continue
        label_file = '/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,'labels.pkl'])
        enc_file = '/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,'enc_vec.npy'])
        if (override or not os.path.isfile(enc_file)) and os.path.isfile(label_file):
            ...
            for file in sorted(os.listdir('/'.join([os.getcwd(),'tumiitgait/Static Occlusion', folder]))):
                if file[-3:]!="pkl" and file[-3:]!="npy":
                    file_name = '/'.join([os.getcwd(), 'tumiitgait/Static Occlusion', folder, file])
                    images.append(cv2.copyMakeBorder((cv2.imread(file_name)[:,:,0]), 0, 0, 20, 20, cv2.BORDER_CONSTANT, (0,0,0)).reshape(160,160,1)/255.)
            ...
            info['/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder])] = enc_vec
            if save:
                np.save('/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder,'enc_vec']),enc_vec)
        elif os.path.isfile(enc_file):
            info['/'.join([os.getcwd(),'tumiitgait/Static Occlusion',folder])] = np.load(enc_file)
    return info
